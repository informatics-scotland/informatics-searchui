    ...
    SearchUIApp.loadConfig('configuration', (data: string | null, error: string | null) => {
      if (data) {
        try {
          ...
          // switch search profile based upon the query string if required
          const queryParams = this.getQueryParameters();
          if (queryParams.businesscenterprofile){
            config.Searcher.businessCenterProfile = queryParams.businesscenterprofile;
          }
